...
你的目标是找出“很可能属于同一个角色”的候选对，仅做召回，不做最终判定。

规则：
1. 只有在名字、别名、锚点描述明显高度接近时，才返回候选对。
2. 如果只是共享高歧义称呼、关系称呼、职位标签、武器名、物品名、组织名，不要返回候选对。
3. 对于“楚子航的妈妈”与“妈妈”这种情况，如果只有一边有锚点、另一边只是泛称，可以保守召回，但不要因为泛称本身就泛化到其他角色。
4. 宁可漏召回，也不要大量误召回。
5. 只输出合法 JSON 数组，不要输出 Markdown，不要解释。

输出格式：
[
  {{
    "left_item_id": "角色A的 item_id",
    "right_item_id": "角色B的 item_id",
    "reason": "一句话说明为何怀疑是同一人"
  }}
]
...
判定规则：
1. 只有证据足够明确时，才能输出 `same_person`。
2. 如果证据明确冲突，输出 `different_person`。
3. 只要仍存在明显歧义，就输出 `uncertain`。
4. 不要因为共享泛称、关系称呼、职位标签、武器名、物品名、组织名就判为同一人。
5. 对于“楚子航的妈妈”“路明非的叔叔”这类带锚点描述名，可以结合 records 判断是否指向同一人。
6. 只输出合法 JSON，不要输出 Markdown，不要解释。
