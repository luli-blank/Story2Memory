...
2. **character**
   ...
   - `character` 中只允许出现以下三类名称：
     ...
     2. 稳定专属代号/外号
     3. 带锚点且可唯一指向的关系描述名
   - 如果角色没有出现明确姓名，但文本能稳定锚定其身份关系或场景身份，请使用**简洁且可区分的锚点式描述名**，例如：【林澈的妈妈】、【赵明的叔叔】、【赵明住院时的护士长】。
...

{
  ...
  "character": [
    {
      "name": "顾远",
      "description": "顾远在睡前浏览论坛灵异帖子，并对帖子内容产生明显警觉。"
    }
  ],
  "ambiguous_character_mentions": [
    {
      "surface_name": "叔叔",
      "description": "该人物在本章中催促赵明出门，并以长辈身份与其互动。",
      "evidence_excerpt": "叔叔在门外喊赵明快点出来。"
    }
  ],
  "special_existence": [
    {
...
8. 示例：
   - 输入候选：麻衣、麻衣队长、麻衣（队长）、周青
   - 正确输出：`canonical_name = 周青`，`aliases = []`
